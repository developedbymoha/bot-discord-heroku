(function() {
  "use strict";
  const { createAPI, pathResolve, creationOptions, createFile, debug } = require('./requirements.js');
  ...
  class Database {
    /** 
    * Database is simple and easy that helps you store and read data in your project
    ...
    */
    constructor(options) {
      ...
      this.type = this.inputPath.endsWith("yml") || options.path.endsWith("yaml") ? "yml" : "json"
      this.path = pathResolve(options.path, this.type)
      this.event = new EventEmitter();
      this.readyInDate = new Date();
      createFile(this.path, this.type == "yml" ? true : false)
      if (this.debug == true) {
      ...
      if (this.API == true) {
        ...
        createAPI(this.inputPath, [], this.debug)
      }
      this.size = this.load().size || 0
      this.cache = this.load() || new Map()
    }
    /**
    ...
    */
    ...
    put(key, value) {
      ...
      let cache = this.load()
      cache.set(key, this.encryption == true ? this.encryptString(value) : value)
    ...
    */
    ...
    all(limit = 0,analysis) {
      ...
      let cache = Array.from(this.load())
      let arr = []
      cache.forEach((key, v) => {
    ...
    */
    ...
    transferDB(db) {
      ...
      let fileData = db.load()
      let localData = this.load()
    ...
    OldToNewDatabase(oldPath) {
      let oldtype = oldPath.endsWith("yml") || oldPath.endsWith("yaml") ? "yml" : "json"
      oldPath = pathResolve(oldPath, oldtype)
      try {
        if (this.type == "yml") {
          return new Map(Object.entries(YAML.parse(readFileSync(oldPath, 'utf8'))))
        } else {
          return new Map(Object.entries(JSON.parse(readFileSync(oldPath))))
        }
      } catch (e) {
        return new Map()
      }
